function showModal(type, id) {
    ...
    $('#theLoaiId').val(id); // Cập nhật ID chất liệu vào input ẩn
    $('#theLoaiForm')[0].reset(); // Reset form khi mở modal
    console.log("id :" + id);

    if (type === 'view' || type === 'edit') {
        // Gọi API để lấy chi tiết chất liệu
        $.get('/the-loai/' + id)
            .done(function (theLoai) {
                $('#myModal h2').text(type === 'view' ? 'Chi tiết thể loại' : 'Cập nhật thể loại');
                $('#tenTheLoai').val(theLoai.tenTheLoai);
                $('#trangThai').val(theLoai.trangThai); // Đặt giá trị trạng thái vào combobox
        ...
        $('#myModal h2').text('Thêm thể loại mới');
...
$(document).ready(function () {
    $('#theLoaiForm').submit(function (event) {
...
function confirmAction() {
    ...
    const theLoaiId = $('#theLoaiId').val();
    let url = '';
    let method = '';

    if (actionType === 'add') {
        url = '/the-loai/them-the-loai';
        method = 'POST';
    } else if (actionType === 'edit') {
        url = '/the-loai/' + theLoaiId;
        method = 'PUT';
    }
