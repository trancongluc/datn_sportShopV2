function showModal(type, id) {
    ...
    $('#deGiayId').val(id); // Cập nhật ID chất liệu vào input ẩn
    $('#deGiayForm')[0].reset(); // Reset form khi mở modal
    console.log("id de giay:" + id);

    if (type === 'view' || type === 'edit') {
        // Gọi API để lấy chi tiết chất liệu
        $.get('/de-giay/' + id)
            .done(function (deGiay) {
                $('#myModal h2').text(type === 'view' ? 'Chi tiết đế giày' : 'Cập nhật đế giày');
                $('#tenDeGiay').val(deGiay.tenDeGiay);
                $('#trangThai').val(deGiay.trangThai); // Đặt giá trị trạng thái vào combobox
        ...
        $('#myModal h2').text('Thêm đế giày mới');
...
$(document).ready(function () {
    $('#deGiayForm').submit(function (event) {
...
function confirmAction() {
    ...
    const deGiayId = $('#deGiayId').val();
    let url = '';
    let method = '';

    if (actionType === 'add') {
        url = '/de-giay/them-de-giay';
        method = 'POST';
    } else if (actionType === 'edit') {
        url = '/de-giay/' + deGiayId;
        method = 'PUT';
    }

    const formData = JSON.stringify({
        tenDeGiay: $('#tenDeGiay').val(),
