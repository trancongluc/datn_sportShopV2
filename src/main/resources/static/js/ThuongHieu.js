function showModal(type, id) {
    ...
    $('#thuongHieuId').val(id); // Cập nhật ID chất liệu vào input ẩn
    $('#thuongHieuForm')[0].reset(); // Reset form khi mở modal
    console.log("id :" + id);

    if (type === 'view' || type === 'edit') {
        // Gọi API để lấy chi tiết chất liệu
        $.get('/thuong-hieu/' + id)
            .done(function (thuongHieu) {
                $('#myModal h2').text(type === 'view' ? 'Chi tiết Thương Hiệu' : 'Cập nhật Thương Hiệu');
                $('#tenThuongHieu').val(thuongHieu.tenThuongHieu);
                $('#trangThai').val(thuongHieu.trangThai); // Đặt giá trị trạng thái vào combobox
        ...
        $('#myModal h2').text('Thêm thể Thương Hiệu');
...
$(document).ready(function () {
    $('#thuongHieuForm').submit(function (event) {
...
function confirmAction() {
    ...
    const thuongHieuId = $('#thuongHieuId').val();
    let url = '';
    let method = '';

    if (actionType === 'add') {
        url = '/thuong-hieu/them-thuong-hieu';
        method = 'POST';
    } else if (actionType === 'edit') {
        url = '/thuong-hieu/' + thuongHieuId;
        method = 'PUT';
    }
